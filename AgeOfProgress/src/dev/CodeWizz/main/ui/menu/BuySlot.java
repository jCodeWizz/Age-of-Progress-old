	...
	public void renderUI(GameContainer gc, Renderer r) {
		if (activated) {
			if (pressed) {
				...
				r.drawText(title, x+w-5-r.getFont().getWidth(title), y+32, 0xffffffff, space);
				r.fillRectUI(x-10+w-r.getFont().getWidth(title), y+32+17, r.getFont().getWidth(title)+10, 2, 0xff2f1d1d, Light.NONE, space);
				
				r.drawText("" + cost, x+w-5-r.getFont().getWidth(title), y+55, 0xffffffff, space);
				r.fillRectUI(x-10+w-r.getFont().getWidth(title), y+55+17, r.getFont().getWidth(title)+10, 2, 0xff2f1d1d, Light.NONE, space);
			}
		} else {
		}
	}
